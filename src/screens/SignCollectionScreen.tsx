...
import { View, Text, Pressable, TextInput, Alert, ScrollView, Image, ActivityIndicator } from "react-native";
...
import { getCollectionByUuid, getCustomerById, signCollection as signCollectionInDB } from "../database/db-collections";
...
export default function SignCollectionScreen({ navigation, route }: Props) {
  ...
  const handleSign = async () => {
    ...
    try {
      ...
      // Reload collection to get the signature
      const signedCollection = await getCollectionByUuid(collection.id);
        ...
        [
          {
            ...
            onPress: async () => {
              if (signedCollection) {
                await emailCollectionReport(signedCollection, customer?.email);
              }
            },
          },
          {
